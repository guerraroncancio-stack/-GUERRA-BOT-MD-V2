...
async function addExif(webpBuffer, packname, author, categories = ["⚔️"], extra = {}) {
    const img = new webp.Image();

    const json = {
        "sticker-pack-id": "guerra-" + randomBytes(16).toString("hex"),
        "sticker-pack-name": `⚔️ ${packname}`,
    ...
    return new Promise((resolve, reject) => {
        ...
        fluent_ffmpeg(src)
            ...
            .addOutputOptions([
                "-vcodec", "libwebp",
                "-vf",
                "scale='min(320,iw)':'min(320,ih)':force_original_aspect_ratio=decrease,pad=320:320:-1:-1:color=0x00000000",
                "-pix_fmt", "rgba",
                "-lossless", "1",
                "-quality", "90",
                "-compression_level", "3",
    ...
    await new Promise((resolve, reject) => {
        fluent_ffmpeg(inPath)
            .addOutputOptions([
                "-vcodec", "libwebp_anim",
                "-vf",
                "fps=15,scale='min(320,iw)':'min(320,ih)':force_original_aspect_ratio=decrease,pad=320:320:-1:-1:color=0x00000000",
                "-pix_fmt", "yuva420p",
                "-loop", "0",
                "-quality", "50",
                "-compression_level", "1",
    ...
    run: async (m, { conn, args }) => {
        try {
            ...
            if (/video/.test(mime)) {
                ...
                if (duration > 7) {
                    await m.react("❌");
                    return m.reply("> ⍰ Video demasiado largo (máx 7s).");
                }
            }
            ...
            if (!buffer) return m.reply("> ⚔ Error al descargar archivo.");
            ...
            const botName = typeof global.botname === "string" ? global.botname : "⚔ GUERRA BOT";
