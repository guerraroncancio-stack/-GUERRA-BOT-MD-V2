...
const conf = {
  ...
  social: {
    channel: '120363427020147321@newsletter'
  }
}
...
global.name = (c) => {
  ...
  if (global.subbotConfig?.[id]) {
    return global.subbotConfig[id].botName || global.botNames[0]
  }

  return global.botNames[
    Math.floor(Math.random() * global.botNames.length)
  ]
}
...
global.img = (c) => {
  ...
  const id = connection?.user
    ? jidNormalizedUser(connection.user.id)
    : null

  let url = global.botImages[0]

  if (connection?.settings?.botImage) {
    url = connection.settings.botImage
  } else if (id && global.subbotConfig?.[id]) {
    url = global.subbotConfig[id].botImage || global.botImages[0]
  } else {
    url = global.botImages[
      Math.floor(Math.random() * global.botImages.length)
    ]
  }
...
global.img2 = (c) => {
  ...
  if (connection?.settings?.botImage) {
    ...
    url = global.subbotConfig[id].botImage || global.botImages2[0]
  } else {
    url = global.botImages2[
      Math.floor(Math.random() * global.botImages2.length)
    ]
  }
...
global.getBuffer = async (url, options = {}) => {
  ...
  try {
    ...
    if (global.bufferCache.has(url)) {
      return global.bufferCache.get(url)
    }
...
global.fecha = d.toLocaleDateString('es-CO', {
  day: 'numeric',
  month: 'numeric',
  year: 'numeric'
})

global.tiempo = d.toLocaleString('es-CO', {
  hour: 'numeric',
  minute: 'numeric',
  hour12: true
})
