...
export async function startSubBot(m, conn, id, { isCode = false, caption = '' } = {}) {
 ...
 if (!state.creds.registered) {
    ...
    try {
        const phoneNumber = String(id).replace(/\D/g, '')

        if (!phoneNumber) {
            throw new Error('Número inválido')
        }

        console.log(chalk.yellow(`┃ [SUB-BOT] SOLICITANDO CÓDIGO PARA ${phoneNumber}`))
        ...
        console.log(chalk.green(`┃ [SUB-BOT] CÓDIGO GENERADO: ${pairingCode}`))

        if (isCode && pairingCode && conn && m) {
            try {
                const msgCode = await conn.sendMessage(
                    m.chat,
                    {
                        text: `
╭━━━〔 ⚔️ GUERRA BOT ⚔️ 〕━━━⬣
...
`
        ...
        console.error(
            chalk.red(`┃ [SUB-BOT] ERROR PAIRING: ${id}`),
            e?.message
        )
